...
impl Into<Event> for WayPoint {
    fn into(self) -> Event {
        Event {
            id: self.id,
            x: format!("{:.6}", self.pos.x_y().0),
            y: format!("{:.6}", self.pos.x_y().1),
        }
    }
}
...
impl Driver {
    fn move_to(&mut self, p: Point<f64>) {
        let d = self.1.previous_point.map(|prev| prev.geodesic_distance(&p)).unwrap_or(0.0);
        self.1.traveled += d;
...
impl StreamHandler<WayPoint> for Driver {
    fn handle(&mut self, p: WayPoint, ctx: &mut Context<Self>) {
        ...
        info!("{}: {:.1}m ({:.1}%)", self.1.current_step, self.1.traveled, pct);
...
fn main() -> std::io::Result<()> {
    ...
    for feature in layer.features().filter(|f| usable_feature(f, did)) {
        ...
        let output = match &opts.output {
            SubCommand::Stdout(cmd) => {
                Publisher::stdout(cmd.pretty)
            }
            SubCommand::Http(cmd) => {
                Publisher::http(&cmd.uri, cmd.insecure)
            }
        };
